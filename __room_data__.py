...
def generate_Dummy_Room_Data(user_id: str, appliances_list: list):
    ...
    for a in loads:
        ...
        # cipher_current_total = encrypt_Text(str(round(current_total, 1)))
        # cipher_power_total = encrypt_Text(str(round(power_total, 1)))

        cipher_v = (str(v)); cipher_f = (str(f))
        cipher_pf = (str(pf)); cipher_i = (str(i))
        cipher_p = (str(p));
        cipher_current_total = (str(round(current_total, 1)))
        cipher_power_total = (str(round(power_total, 1)))
